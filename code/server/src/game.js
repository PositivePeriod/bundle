...
class ServerGame {
    constructor(io, socketIDA, socketIDB) {
        this.id = nanoid();
        this.sockets = [
            io.sockets.sockets.get(socketIDA),
            io.sockets.sockets.get(socketIDB),
        ];
        ...
        io.to(this.id).emit("startGame", { id: this.id, players: [this.sockets[0].data.playerName, this.sockets[1].data.playerName] });
    ...
    async play() {
        ...
        while (!this.board.ended) {
            ...
            } else { var { piece, dir } = await this.choose(I, "move", moves); }
